        ...
        <main class="col-md-9 offset-1 ms-sm-auto">
                        ...
                        <div class="modal-body">
                            <form:form action="/category" method="post" modelAttribute="categoryDto">
                                <div class="mb-3">
                                    ...
                                    <form:input type="text" class="form-control" id="name-uz-add" path="nameUz"
                                                placeholder="Nomi(Uz)"/>
                                </div>
                                <div class="mb-3">
                                    <label for="name-ru-add" class="form-label">Nomi(Ru)</label>
                                    <form:input type="text" class="form-control" id="name-ru-add" path="nameRu"
                                                placeholder="Nomi(Ru)"/>
                                </div>
            ...
            <table class="table mb-4">
                ...
                <c:forEach items="${categories}" var="category" varStatus="loop">
                    <tr>
                        ...
                        <td id="name-uz">${category.nameUz}</td>
                        <td id="name-ru">${category.nameRu}</td>
                        <td id="category-id" hidden="hidden">${category.id}</td>
                        <td>
            ...
            <div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="updateModalLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="updateModalLabel">Kategoriya Yangilash</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form:form action="/category/update" method="post" id="update-form"
                                       modelAttribute="categoryWithIdDto">
                                <div class="mb-3">
                                    <label for="name-uz-up" class="form-label">Nomi(Uz)</label>
                                    <form:input type="text" class="form-control" id="name-uz-up" placeholder="Nomi(Uz)"
                                                path="nameUz"/>
                                </div>
                                <div class="mb-3">
                                    <label for="name-ru-up" class="form-label">Nomi(Ru)</label>
                                    <form:input type="text" class="form-control" id="name-ru-up" name="nameRu"
                                                placeholder="Nomi(Ru)" path="nameRu"/>
                                </div>
                                <button type="submit" class="btn btn-primary">Saqlash</button>
                            </form:form>
                        </div>
...
<script>
    ...
    function getFields(element) {
        ...
        inputElement.value = categoryIdValue;
